...
@section('script')
    ...
    <script>
        $(function() {
            $.ajax({
                url: "{{ route('loan.monthly_request_loans') }}",
                type: "GET",
                dataType: "json",
                    ...
                    $('#datatable').DataTable({
                        ...
                        columnDefs: [
                            ...
                            },
                            {
                                ...
                                targets: 6,
                                render: function(data, type, row) {
                                    return row.approved_sum;
                                }
                            }, {
                                targets: 7,
                                render: function(data, type, row) {
                                    var total = row.loan.loan;
                                    var interest = row.loan.interest;
                                    var payed = parseInt(row.approved_sum);

                                    var sum = total + interest;
                                    return sum - payed;
                                }
                            }, {
                                targets: 8,
    ...
    <script>
        $(document).on('click', '.rejectModel', function() {
            var id = $(this).find('span').data('id');
            var route = "{{ route('loan.monthly_loan_reject', ['id' => ':id']) }}";
            route = route.replace(':id', id);
